...
public class GoogleShoppingParser implements ISeleniumParser {
    ...
    @Override
    public List<OnlineShoppingItemDTO> parse(@NonNull final WebDriver webDriver) {

        var root = Utils.translateWebElementException(() ->
                webDriver.findElement(By.className("sh-pr__product-results-grid")));
        if(root == null) {
            log.debug("Looking for class name: {}, actual DOM: {}", "sh-pr__product-results-grid",Utils.reflectionToString(webDriver.getPageSource()));
        ...
        if(items == null) {
            log.debug("Looking for class name: {}, actual DOM: {}", "sh-dgr__content", Utils.reflectionToString(webDriver.getPageSource()));
            return new ArrayList<>(); // swallow WebDriver exceptions and assume no result found.
        }
